  ...
  try {
    ...
    for (const item of studyPlan.items) {
      ...
      if (item.type === 'weak_topic') {
        ...
        matches =
          activityTags.some((tag) => tag.toLowerCase().includes(topicLower)) ||
          activityTitle.includes(topicLower);
      ...
      if (matches) {
        ...
        updated = true;
        logger.info({ userId, itemTopic: item.topic, activitiesCount: item.activitiesCount }, 'Study plan item progress updated');
      }
    }
    ...
    if (updated) {
      ...
      const allCompleted = studyPlan.items.every(
