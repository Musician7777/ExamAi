 ...
 * NOTE: This does NOT evaluate test cases — that is done by the real Piston executor.
 * This only provides a generic analysis skeleton so the UI has something to display.
 */
export function getMockCodeAnalysisResponse(config) {
  const { code } = config || {};
  const isStarter =
  ...
  if (isStarter) {
    return {
      feedback: 'Please implement your solution before submitting.',
      timeComplexity: '—',
      spaceComplexity: '—',
      suggestions: ['Implement the solution logic first'],
    };
  }

  return {
    feedback:
      'Solution submitted. AI analysis is temporarily unavailable — check the test results above for pass/fail status.',
    timeComplexity: 'Analysis unavailable',
    spaceComplexity: 'Analysis unavailable',
    suggestions: ['Review any failing test cases and check edge conditions'],
  };
}
