...
/**
 * Strip internal error details from API responses in production.
 * Only includes the `details` field in development for debugging.
 * Prevents leaking stack traces, internal service names, or API key info.
 *
 * @param {Object} response — Error response object, may contain a `details` key
 * @returns {Object} — Sanitized response (details removed in production)
 */
export function sanitizeErrorResponse(response) {
  if (process.env.NODE_ENV === 'production') {
    const { details, ...safe } = response;
    return safe;
  }
  return response;
}
