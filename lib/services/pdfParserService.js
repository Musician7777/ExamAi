...
export async function parsePDF(fileBuffer) {
  try {
    ...
    // Using pdf-parse directly without specific path - fixes the import error
    const pdfModule = await import('pdf-parse');
    const pdfParse = pdfModule.default || pdfModule;
    
    // Set up options to prevent pdf-parse from trying to read test files
    const options = {
      max: 0, // No page limit
      version: 'v2.0.550', // Prevent version checking
    };
