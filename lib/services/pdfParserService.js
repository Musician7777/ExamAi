...
export async function parsePDF(fileBuffer) {
  try {
    ...
    logger.error({ err: error }, 'PDF parse error');
    return {
      success: false,
      text: '',
      pageCount: 0,
...
export function analyzeExamPattern(text) {
  ...
  const questionPatterns = [/(?:Q|Question)\s*\.?\s*(\d+)/gi, /(\d+)\s*[.)]\s+/g, /(?:^|\n)\s*(\d+)\s*\.\s+\S/gm];
  ...
  // Detect sections
  const sectionKeywords = ['Section', 'Part', 'SECTION', 'PART', 'Paper', 'PAPER', 'Module', 'MODULE'];
  const sections = [];
