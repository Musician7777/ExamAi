  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (isRetryableGeminiError(error) && attempt < retries) {
        ...
        console.warn(
          `Key ${apiKey.slice(-6)} temporarily unavailable. Retry ${attempt + 1}/${retries} in ${delay}ms...`
        );
...
export async function generateWithFailover(prompt) {
  ...
  for (let i = 0; i < API_KEYS.length; i++) {
    try {
      // eslint-disable-next-line no-console -- API key failover debug logging
      console.debug(`Trying Gemini API key ${i + 1}/${API_KEYS.length}...`);
      return await tryWithKey(API_KEYS[i], prompt);
    } catch (error) {
      lastError = error;
      if (isRetryableGeminiError(error) && i < API_KEYS.length - 1) {
        console.warn(`Key ${i + 1} exhausted. Switching to key ${i + 2}...`);
      } else if (i < API_KEYS.length - 1) {
        console.warn(`Key ${i + 1} failed (${error.message}). Trying key ${i + 2}...`);
      }
    }
  }
  throw lastError;
