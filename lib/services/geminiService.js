...
import { cacheWrap, CACHE_KEYS, CACHE_TTL, cacheGet, cacheSet } from '@/lib/services/redisCacheService';
...
// Try all available API keys with failover and caching
export async function generateWithFailover(prompt, options = {}) {
  const { cacheTTL = CACHE_TTL.EXTENDED, skipCache = false } = options;

  // Check if we should cache this request (for non-mission-critical generation)
  if (!skipCache && options.cacheKey) {
    const cached = await cacheGet(options.cacheKey);
    if (cached) {
      logger.debug({ key: options.cacheKey }, '[Gemini] Cache HIT');
      // Return a mock response object that mimics the Gemini response structure
  ...
  for (let i = 0; i < API_KEYS.length; i++) {
    try {
      ...
      const result = await tryWithKey(API_KEYS[i], prompt);

      // Cache successful response text (not the full object which loses methods on deserialization)
      if (!skipCache && options.cacheKey && result) {
        const text = result.response.text();
        await cacheSet(options.cacheKey, text, cacheTTL);
        logger.debug({ key: options.cacheKey, ttl: cacheTTL }, '[Gemini] Cached response text');
      }
