...
export function FetchExamModal({ isOpen, onClose, onUseConfig, onSavePreset, mode = 'exam' }) {
  ...
  const apiType =
    mode === 'interview' ? 'fetch-interview-config' : mode === 'coding' ? 'fetch-coding-config' : 'fetch-exam-config';
    ...
    >
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          <Button className="w-full gap-2" variant="brand" disabled={!examName.trim() || loading} onClick={handleFetch}>
            {loading ? (
              <>
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                {mode === 'exam' && (
                  <>
                    ...
                    <span>
                      Time: <strong>{result.timeLimit} min</strong>
                    </span>
                    <span>
                      Type: <strong>{result.questionType || 'MCQ'}</strong>
                    </span>
                    <span>
                      Negative: <strong>{result.negativeMarking || 0}</strong>
                    </span>
                    <span>
                      Sections: <strong>{result.sections?.length || '—'}</strong>
                    </span>
                    <span>
                      Marks/Q: <strong>{result.marksPerQuestion || '—'}</strong>
                    </span>
                  </>
                )}
                {mode === 'interview' && (
                  <>
                    <span>
                      Type: <strong>{result.interviewType}</strong>
                    </span>
                    <span>
                      Difficulty: <strong>{result.difficulty}</strong>
                    </span>
                    <span>
                      Questions: <strong>{result.questionCount}</strong>
                    </span>
                    <span>
                      Tone: <strong>{result.tone}</strong>
                    </span>
                    <span>
                      Role: <strong>{result.role}</strong>
                    </span>
                    <span>
                      Topics: <strong>{result.topics?.length || 0}</strong>
                    </span>
                  </>
                )}
                {mode === 'coding' && (
                  <>
                    <span>
                      Problems: <strong>{result.problems?.length || 0}</strong>
                    </span>
                    <span>
                      Difficulty: <strong>Mixed</strong>
                    </span>
                  </>
                )}
              </div>
...
export function SavedPresetsSection({ children, count }) {
  ...
  return (
    <>
      <div className="text-sm font-medium text-muted-foreground mt-6 mb-3">💾 Your Saved Presets ({count})</div>
      {children}
    </>
  );
}
