...
export function useAnalytics() {
    ...
    const scores = useMemo(() => activities.filter(a => a.totalMarks > 0).map(a => Math.round((a.score / a.totalMarks) * 100)).reverse(), [activities]);
    const labels = useMemo(() => scores.map((_, i) => `Activity ${i + 1}`), [scores]);

    // Type-wise scores
    const typeScores = useMemo(() => buildTypeScores(activities), [activities]);
    const topicLabels = useMemo(() => Object.keys(typeScores).length > 0 ? Object.keys(typeScores) : ['Coding', 'Exams', 'Interviews'], [typeScores]);
    const topicValues = useMemo(() => topicLabels.map(t => typeScores[t] ? Math.round(typeScores[t].total / typeScores[t].count) : 0), [topicLabels, typeScores]);
    ...
    const deepTopicValues = useMemo(() => deepTopicEntries.map(([_, v]) => Math.round(v.total / v.count)), [deepTopicEntries]);
    const deepTopicCounts = useMemo(() => deepTopicEntries.map(([_, v]) => v.count), [deepTopicEntries]);
    const weakTopics = useMemo(() => deepTopicEntries.filter(([_, v]) => (v.total / v.count) < 50).map(([k, v]) => ({ name: k, avg: Math.round(v.total / v.count), count: v.count })), [deepTopicEntries]);
    const strongTopics = useMemo(() => deepTopicEntries.filter(([_, v]) => (v.total / v.count) >= 75).map(([k, v]) => ({ name: k, avg: Math.round(v.total / v.count), count: v.count })), [deepTopicEntries]);

    // Difficulty-based scores
    const easyScores = useMemo(() => activities.filter(a => a.details?.difficulty === 'easy' || (a.totalMarks > 0 && (a.score / a.totalMarks) >= 0.8)), [activities]);
    const medScores = useMemo(() => activities.filter(a => a.details?.difficulty === 'medium' || (a.totalMarks > 0 && (a.score / a.totalMarks) >= 0.5 && (a.score / a.totalMarks) < 0.8)), [activities]);
    const hardScores = useMemo(() => activities.filter(a => a.details?.difficulty === 'hard' || (a.totalMarks > 0 && (a.score / a.totalMarks) < 0.5)), [activities]);
