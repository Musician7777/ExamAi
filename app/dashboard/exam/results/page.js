...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      setData(parsed); // eslint-disable-line react-hooks/set-state-in-effect -- SSR-safe sessionStorage init
        ...
        <div className="space-y-4">
          {results.map((r, i) => (
            <Card
              ...
              className={cn(
                'p-6 border-l-4',
                r.userAnswer === null ? 'border-l-muted' : r.isCorrect ? 'border-l-success' : 'border-l-destructive'
              )}
            >
              ...
              <div className="space-y-2 mb-4 text-sm bg-secondary/30 p-4 rounded-lg">
                {r.userAnswer !== null && !r.isCorrect && (
                  <p className="text-destructive">
                    Your answer: <span className="font-semibold">{r.options[r.userAnswer]}</span>
                  </p>
                )}
                <p className="text-success">
                  Correct answer: <span className="font-semibold">{r.options[r.correct]}</span>
                </p>
              </div>
