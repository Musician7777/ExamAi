...
export default function UploadPage() {
    ...
    const handleFile = (f) => { if (f) { setFile(f); setError(null); setAnalysis(null); } };

    const handleProcess = async () => {
        if (!file) return;
        setProcessing(true); setError(null); setCurrentStep(0);
        try {
            await new Promise(r => setTimeout(r, 500)); setCurrentStep(1);
            ...
            setCurrentStep(3); await new Promise(r => setTimeout(r, 800));
            ...
            } else { setProcessing(false); setError('PDF parsed but exam generation requires a Gemini API key.'); }
        } catch (err) { console.error('Upload error:', err); setError(err.message || 'Failed to process file'); setProcessing(false); }
    };
    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <p className="text-muted-foreground mt-1">Upload a previous year question paper and our AI will learn its pattern to generate a similar exam.</p>
            </div>
                ...
                <>
                    <Card
                        className={cn(
                            ...
                            dragOver ? "border-indigo-500 bg-indigo-500/5" : file ? "border-emerald-500/50 bg-emerald-500/5" : "hover:border-indigo-500/30"
                        )}
                        ...
                        <Card className="p-6 space-y-3">
                            ...
                            <div className="grid sm:grid-cols-2 gap-3 text-sm">
                                <div className="flex justify-between p-2 rounded-md bg-secondary/50">
                                    <span className="text-muted-foreground">Pages</span><strong>{analysis.pageCount}</strong>
                                </div>
                                <div className="flex justify-between p-2 rounded-md bg-secondary/50">
                                    <span className="text-muted-foreground">Detected Questions</span><strong>{analysis.detectedQuestions || 'N/A'}</strong>
                                </div>
                                <div className="flex justify-between p-2 rounded-md bg-secondary/50">
                                    <span className="text-muted-foreground">Sections</span><strong>{analysis.detectedSections?.join(', ') || 'None detected'}</strong>
                                </div>
                                <div className="flex justify-between p-2 rounded-md bg-secondary/50">
                                    <span className="text-muted-foreground">Multiple Choice</span><strong>{analysis.patterns?.hasMultipleChoice ? 'Yes' : 'No'}</strong>
                                </div>
                            </div>
                        </Card>
                    )}
                        ...
                        {steps.map((step, i) => (
                            <div key={i} className="flex items-center gap-3">
                                <div className={cn(
                                    ...
                                    i === currentStep ? "bg-indigo-500/20 text-indigo-400" : "bg-secondary text-muted-foreground"
                                )}>
                                    {i < currentStep ? <Check className="h-4 w-4" /> : i === currentStep ? <Loader2 className="h-4 w-4 animate-spin" /> : <span>{i + 1}</span>}
                                </div>
