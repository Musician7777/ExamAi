...
function Conversation({ className, children, ...props }) {
  ...
  return (
    <ConversationContext.Provider value={{ isAtBottom, scrollToBottom }}>
      <div className={cn('relative flex h-full flex-col overflow-hidden', className)} {...props}>
        <div ref={scrollRef} className="flex-1 overflow-y-auto" onScroll={checkIfAtBottom}>
          {children}
        </div>
      </div>
...
function ConversationContent({ className, children, ...props }) {
  return (
    <div className={cn('flex min-w-0 flex-col gap-2 p-6 pb-2', className)} {...props}>
      {children}
    </div>
  );
}
...
function ConversationEmptyState({ icon, title, description, className, children, ...props }) {
  return (
    <div
      className={cn('flex flex-1 flex-col items-center justify-center gap-3 py-12 text-center', className)}
      {...props}
    >
  ...
  return (
    <div className="absolute bottom-2 left-1/2 -translate-x-1/2 z-10">
      <Button
        variant="secondary"
        size="sm"
        className={cn('rounded-full shadow-lg border text-xs gap-1 h-7 px-3', className)}
