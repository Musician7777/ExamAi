...
function useBarAnimator(state, barCount, speed = 100) {
  ...
  useEffect(() => {
    ...
    if (state !== 'connecting' && state !== 'initializing') {
      setHighlighted(new Set()); // eslint-disable-line react-hooks/set-state-in-effect -- cleanup on state change
      return;
    }
...
function BarVisualizer({
  ...
  const [barHeights, setBarHeights] = useState(() => Array.from({ length: barCount }, () => minHeight));
      ...
      const newHeights = Array.from({ length: barCount }, (_, i) => {
        ...
        switch (state) {
          ...
          case 'initializing': {
            ...
            return isHighlighted ? minHeight + (maxHeight - minHeight) * 0.6 : minHeight + 4;
          }
          default:
    ...
    (index, height) => {
      ...
      switch (state) {
        ...
        case 'initializing': {
          ...
          return isHighlighted ? `hsl(239, 84%, 67%)` : `hsl(239, 25%, 25%)`;
        }
        default:
  ...
  return (
    <div
      className={cn('flex items-end gap-[2px] w-full', centerAlign ? 'justify-center' : 'justify-between', className)}
      role="img"
